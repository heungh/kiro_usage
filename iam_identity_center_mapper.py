...
class IAMIdentityCenterMapper:
            ...
            while True:
                ...
                for user in response.get('Users', []):
                    user_info = {
                        ...
                        'email': user.get('Emails', [{}])[0].get('Value', '') if user.get('Emails') else '',
    ...
    def get_cache_stats(self) -> Dict:
        ...
        iam_users = len([u for u in self.user_cache.values() if u.get('source') == 'iam_identity_center'])
        fallback_users = len([u for u in self.user_cache.values() if u.get('source') == 'fallback'])
        
        return {
            'total_users': total_users,
            'iam_users': iam_users,
            'fallback_users': fallback_users,
            'identity_store_connected': self.identity_store_client is not None and self.identity_store_id is not None
        }
...
def create_user_mapping_interface():
    ...
    with col1:
        connection_status = "🟢 연결됨" if stats['identity_store_connected'] else "🔴 미연결"
    ...
    if st.button("🔍 Identity Center 사용자 전체 조회"):
        with st.spinner("IAM Identity Center에서 사용자 목록 조회 중..."):
            ...
            if all_users:
                ...
                display_columns = ['username', 'display_name', 'email', 'first_name', 'last_name']
                st.dataframe(
    ...
    with st.expander("🔍 특정 사용자 검색"):
        search_username = st.text_input("사용자명으로 검색", placeholder="bedrock_hheungsu1")
    ...
    if mapper.user_cache:
        ...
        for user_id, info in mapper.user_cache.items():
            cache_data.append({
                ...
                'LastUpdated': time.strftime('%Y-%m-%d %H:%M', time.localtime(info.get('cached_at', 0)))
            })
...
if __name__ == "__main__":
    ...
    # bedrock_hheungsu1 사용자 검색
    print("\n=== bedrock_hheungsu1 사용자 검색 ===")
    user_info = mapper.search_user_by_username("bedrock_hheungsu1")
    if user_info:
